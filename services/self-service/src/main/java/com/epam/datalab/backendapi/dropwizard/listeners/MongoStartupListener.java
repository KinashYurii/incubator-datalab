...
public class MongoStartupListener implements ServerLifecycleListener {
    ...
    public void serverStarted(Server server) {
        ...
        List<EndpointDTO> endpoints = endpointDAO.getEndpoints();
        if (userRoleDao.findAll().isEmpty()) {
            ...
            List<CloudProvider> connectedClouds = getConnectedClouds(endpoints);
            log.info("Check for connected endpoints:\n num of connected endpoints: {} \n: connected cloud:{} "
                    , getConnectedEndpoints(endpoints), connectedClouds);
            connectedClouds.forEach(claudProvider -> populatingRoles.addAll(getRoles(claudProvider)));
    ...
    private List<CloudProvider> getConnectedClouds(List<EndpointDTO> endpoints) {
        return endpoints.stream()
                .map(EndpointDTO::getCloudProvider)
                .collect(Collectors.toList());
    }

    private int getConnectedEndpoints(List<EndpointDTO> endpoints) {
